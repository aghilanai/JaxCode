#!/usr/bin/env python3
"""Add Colab-only pip install of jax-judge to all notebooks that use jax_judge."""
...
def already_has_install(nb: dict) -> bool:
    for cell in nb.get("cells", []):
        src = cell.get("source", [])
        flat = "".join(src) if isinstance(src, list) else str(src)
        if MARKER in flat and "jax-judge" in flat:
            return True
    return False
